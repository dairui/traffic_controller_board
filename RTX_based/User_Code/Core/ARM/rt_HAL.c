 ...
 *----------------------------------------------------------------------------
 *      Name:    RT_HAL.C
 *      Purpose: Hardware Abstraction Layer for ARM
 *      Rev.:    V4.11
 *----------------------------------------------------------------------------
...
Switch_task
        ...
        LDMIAEQ R0!,{R4-R11}            ; Restore Reduced Context
        LDMIAEQ R0!,{R1,LR}
        STMDBEQ SP,{R0}                 ; Set User SP
        LDMDBEQ SP,{SP}^
        MSREQ   SPSR_cxsf,R1            ; Set User CPSR
        MOVSEQ  PC,LR                   ; RETI
...
void os_init_stack (P_TCB p_TCB, FUNCP task_body) {
  ...
  if (p_TCB->full_ctx) {
    ...
    stk[14] = ((U32)task_body & 1) ? (INITIAL_CPSR | CPSR_T_BIT) : INITIAL_CPSR;
