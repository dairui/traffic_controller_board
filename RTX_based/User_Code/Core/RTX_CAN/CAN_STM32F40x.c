...
static CAN_ERROR CAN_hw_set_baudrate (U32 ctrl, U32 baudrate)  {
  ...
  if (baudrate <= 1000000)  {
    ...
    CAN_set_timing(ctrl,  4, 2, 3, brp);
		...
		//CAN_set_timing(ctrl,  12, 8, 3, 20);   //  baudrate 设置也就无效了
...
CAN_ERROR CAN_hw_init (U32 ctrl, U32 baudrate)  {
  ...
  CANx->MCR = ((1 << 4) | (1 << 0));  /* Init mode, disable automatic        */
...
//   CANx->BTR &= ~((1U << 31) | (1 << 30)); 
//   CANx->BTR |=  (testmode & ((1U << 31) | (1 << 30)));    // by_lm
	  CANx->BTR &= ~((1U << 31) | (1 << 30)); 
    CANx->BTR |=   ((0 << 31) | (0 << 30));
...
//     return CAN_OBJECTS_FULL_ERROR;
  ...
  filter_number_bit_pos = (uint32_t)0x00000001;

  /* Initialisation mode for the filter */
  CAN1->FMR |= (uint32_t)0x00000001;

  /* Filter Deactivation */
  CAN1->FA1R &= ~(uint32_t)0x00000001;
	...
	((0x0000FFFF & (uint32_t)0x0000) << 16) | (0x0000FFFF & (uint32_t)0x0000);
